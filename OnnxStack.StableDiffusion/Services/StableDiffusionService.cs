...
namespace OnnxStack.StableDiffusion.Services
{
    ...
    public sealed class StableDiffusionService : IStableDiffusionService
    {
        ...
        /// <param name="progressCallback">The callback used to provide progess of the current InferenceSteps.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>The diffusion result as <see cref="SixLabors.ImageSharp.Image<Bgr24>"/></returns>
        public async Task<Image<Bgr24>> GenerateAsBufferAsync(ModelOptions model, PromptOptions prompt, SchedulerOptions options, Action<int, int> progressCallback = null, CancellationToken cancellationToken = default)
        {
            return await GenerateAsync(model, prompt, options, progressCallback, cancellationToken)
                .ContinueWith(t => t.Result.ToBuffer(), cancellationToken)
                .ConfigureAwait(false);
        }
